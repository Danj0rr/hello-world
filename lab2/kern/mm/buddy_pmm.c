...
//每个节点入如果size不为0，则表明这个节点在管理内存，longest表明这个节点管理内存的大小
struct buddy2 {
...
void buddy2_new(int size) {
    ...
    int i;
    nr_block = 0;
    if (size < 1 || !IS_POWER_OF_2(size))//不存在或未对其，再见
        return;

    root[0].size = size;//根节点管理所用的空闲页表
    node_size = size * 2;//这里*2是为了下面的循环初始化root的longest
...
buddy_init_memmap(struct Page* base, size_t n)
{
    ...
    for (; p != base + n; p++)
    {
        ...
        p->property = 1;
        set_page_ref(p, 0);
        SetPageProperty(p);//将每个物理页设置为空
        list_add_before(&free_list, &(p->page_link));//将每个page链入空表
    }
    nr_free += n;
    int allocpages = UINT32_ROUND_DOWN(n);//将size按2的幂次向下取整
